...
type Command =
  | { type: "initialize" | "random" }
  | { type: "validate" | "solve"; facelets: string };
type WorkerResponse =
...
class LocalSolver {
  ...
  initialize(): Promise<void> {
    ...
    if (!this.initPromise)
      this.initPromise = this.request<void>({ type: "initialize" }).then(() => {
        this.ready = true;
      });
    return this.initPromise;
  }

  validate(facelets: string): Promise<ValidationResponse> {
    return this.request({ type: "validate", facelets });
  }
  async solve(facelets: string): Promise<SolvedResponse> {
    await this.initialize();
    return this.request({ type: "solve", facelets });
  }
  randomState(): Promise<string> {
    return this.request({ type: "random" });
  }
  ...
  cancel(): void {
    ...
    for (const pending of this.pending.values())
      pending.reject(new Error("Расчёт отменён."));
    this.pending.clear();
  }
}
