...
import { isRequestCurrent, randomSync, SOLVED, solveSync, validatePhysical } from './core';
...
function swap(state: string, ...pairs: Array<[number, number]>): string {
  const values = [...state];
  for (const [left, right] of pairs) [values[left], values[right]] = [values[right], values[left]];
  return values.join('');
}
...
describe('offline cube solver', () => {
  ...
  it('solves 100 deterministic legal states', { timeout: 120_000 }, () => {
    for (let index = 0; index < 100; index += 1) {
      ...
      const solved = result.moves.reduce((cube, move) => applyMove(cube, move.notation), state);
