...
function stickerSpecs(): StickerSpec[] {
  ...
  const add = (
    ...
    specs.push({
      face,
      index: FACE_NAMES.indexOf(face) * 9 + row * 3 + column,
      position,
      rotation,
      cubiePosition,
    });
...
const CUBIE_POSITIONS: Vector3[] = [-1, 0, 1].flatMap((x) =>
  [-1, 0, 1].flatMap((y) => [-1, 0, 1].map((z) => [x, y, z] as Vector3)),
);
...
function CubieWithStickers({
  ...
  return (
    <group position={position}>
      <Cubie />
      {(STICKERS_BY_CUBIE.get(positionKey(position)) ?? []).map((sticker) => {
        ...
        return (
          <mesh
            ...
            onClick={(event) => {
              ...
              if (editable && selectedColor)
                onStickerChange?.(sticker.index, selectedColor);
            }}
          >
            <planeGeometry args={[0.76, 0.76]} />
            <meshStandardMaterial
              color={color === "X" ? "#334155" : FACE_COLORS[color]}
              roughness={0.42}
              toneMapped={false}
            />
          </mesh>
...
const FACE_NORMALS: Record<FaceName, Vector3> = {
  U: [0, 1, 0],
  R: [1, 0, 0],
  F: [0, 0, 1],
  D: [0, -1, 0],
...
const NORMAL_TO_FACE = new Map(
  Object.entries(FACE_NORMALS).map(([face, normal]) => [
    positionKey(normal),
    face as FaceName,
  ]),
);
const STICKER_BY_CUBIE_AND_FACE = new Map(
  STICKERS.map((sticker) => [
    `${positionKey(sticker.cubiePosition)}|${sticker.face}`,
    sticker,
  ]),
);
...
export function applyMove(facelets: string, move?: string): string {
  ...
  for (let turn = 0; turn < turns; turn += 1) {
    ...
    STICKERS.forEach((source) => {
      if (
        source.cubiePosition[
          definition.axis === "x" ? 0 : definition.axis === "y" ? 1 : 2
        ] !== definition.layer
      )
        return;
      const cubie = rotateVector(
        source.cubiePosition,
        definition.axis,
        direction,
      );
      const normal = rotateVector(
        FACE_NORMALS[source.face],
        definition.axis,
        direction,
      );
      const face = NORMAL_TO_FACE.get(positionKey(normal));
      const target = face
        ? STICKER_BY_CUBIE_AND_FACE.get(`${positionKey(cubie)}|${face}`)
        : undefined;
...
>) {
  ...
  const target = definition
    ? (definition.direction * direction * turns * Math.PI) / 2
    : 0;
  useEffect(() => {
    ...
    if (!isReplay && rotationRef.current)
  ...
  useFrame((_, delta) => {
    ...
    rotationRef.current.rotation[definition.axis] =
      phase.current === "rewind" ? target * (1 - eased) : target * eased;
    if (progress.current === 1 && phase.current === "rewind") {
  ...
  const moving = definition
    ? CUBIE_POSITIONS.filter(
        (position) => position[axisIndex] === definition.layer,
      )
    : [];
  const staticCubies = definition
    ? CUBIE_POSITIONS.filter(
        (position) => position[axisIndex] !== definition.layer,
      )
    : CUBIE_POSITIONS;
  const props = {
    facelets: facelets ?? "X".repeat(54),
    selectedColor,
    onStickerChange,
  };
  return (
...
export function Cube3D({
  facelets = "X".repeat(54),
  selectedColor,
  onStickerChange,
  compact = false,
  cameraPosition = [5.4, 4.4, 5.4],
  fullscreenOverlay,
  activeMove,
  animationKey,
  replayToken,
  ...
  return (
    <div
      ref={containerRef}
      className={`${fullscreen ? "h-screen rounded-none" : compact ? "h-[360px]" : "h-[460px] sm:h-[560px]"} relative overflow-hidden rounded-[18px] border border-white/[.06] bg-[#06070a] shadow-inner`}
    >
      ...
      <Canvas camera={{ position: cameraPosition, fov: 42 }}>
        ...
        />
        {allowOrbit && (
          <OrbitControls enablePan={false} minDistance={5.4} maxDistance={10} />
        )}
      </Canvas>
      {fullscreen && fullscreenOverlay && (
        <div className="absolute left-4 top-4 z-10 max-h-[calc(100vh-2rem)] overflow-auto rounded-2xl border border-white/10 bg-[#0b0c11]/90 p-3 shadow-2xl backdrop-blur-xl">
          {fullscreenOverlay}
        </div>
      )}
      {activeMove && (
        <div className="pointer-events-none absolute left-1/2 top-4 flex -translate-x-1/2 items-center gap-2 rounded-full border border-violet-300/20 bg-violet-500/20 px-4 py-2 text-sm font-bold text-violet-100 shadow-[0_8px_30px_rgba(124,92,255,.3)] backdrop-blur-xl">
          <span className="text-2xl leading-none">
            {activeMove.includes("'") ? "↺" : "↻"}
          </span>
          <span>
            Ход {activeMove}
            {activeMove.includes("2") ? " × 2" : ""}
          </span>
        </div>
      )}
      <button
        ...
        className="absolute right-3 top-3 rounded-xl border border-white/10 bg-black/40 p-2.5 text-white/65 backdrop-blur hover:bg-white/10 hover:text-white"
        aria-label={
          fullscreen
      ...
      {allowOrbit && (
        <div className="pointer-events-none absolute bottom-4 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border border-white/[.07] bg-black/35 px-3 py-1.5 text-[10px] font-medium text-white/40 backdrop-blur">
