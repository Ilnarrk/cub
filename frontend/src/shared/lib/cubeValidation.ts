...
const VALID_EDGES = new Set(
  ["UR", "UF", "UL", "UB", "DR", "DF", "DL", "DB", "FR", "FL", "BL", "BR"].map(
    signature,
  ),
);
const VALID_CORNERS = new Set(
  ["URF", "UFL", "ULB", "UBR", "DFR", "DLF", "DBL", "DRB"].map(signature),
);
...
function pieceIssues(
  ...
  positions.forEach((indices) => {
    ...
    if (!valid.has(value))
      issues.push({
        code: `invalid-${kind}`,
        message: `Недопустимая комбинация цветов ${kind}.`,
        indices,
      });
  ...
  seen.forEach((entries) => {
    if (
      entries.length > 1 &&
      valid.has(signature(entries[0].map((index) => facelets[index]).join("")))
    )
      issues.push({
        code: `duplicate-${kind}`,
        message: `Одинаковая деталь ${kind} указана несколько раз.`,
        indices: entries.flat(),
      });
  });
  return issues;
}
...
export function validateCubeLocally(facelets: string): CubeIssue[] {
  if (facelets.length !== 54)
    return [
      {
        code: "length",
        message: "Нужно заполнить все 54 стикера.",
        indices: [],
      },
    ];
  ...
  if (emptyCount > 0)
    return [
      {
        code: "incomplete",
        message: `Раскрасьте все стикеры: осталось ${emptyCount}.`,
        indices: [],
      },
    ];
  const issues: CubeIssue[] = [];
  FACE_NAMES.forEach((color: FaceName) => {
    const indices = [...facelets].flatMap((value, index) =>
      value === color ? [index] : [],
    );
    if (indices.length !== 9)
      issues.push({
        code: `count-${color}`,
        message: `Цвет «${color}»: ${indices.length} из 9.`,
        indices: indices.length > 9 ? indices : [],
      });
  });
  issues.push(...pieceIssues(facelets, EDGE_POSITIONS, VALID_EDGES, "ребра"));
  issues.push(
    ...pieceIssues(facelets, CORNER_POSITIONS, VALID_CORNERS, "угла"),
  );
  return issues;
}
...
export function normalizeFaceletsByCentres(facelets: string): string | null {
  ...
  if (
    new Set(centres).size !== FACE_NAMES.length ||
    centres.some((colour) => !FACE_NAMES.includes(colour as FaceName))
  )
    return null;
  const mapping = new Map(
    centres.map((colour, index) => [colour, FACE_NAMES[index]]),
  );
