...
class ApiClient {
  ...
  async solveCube(request: SolveRequest): Promise<SolveResponse> {
    return (
      await this.client.post<SolveResponse>("/solve", request, {
  ...
  async validateCube(request: SolveRequest): Promise<ValidationResponse> {
    return (await this.client.post<ValidationResponse>("/validate", request))
      .data;
  }
  ...
  async getSolveJob(jobId: string): Promise<SolveResponse> {
    return (
      await this.client.get<SolveResponse>(`/solve-jobs/${jobId}`, {
        validateStatus: (s) => s < 300,
      })
    ).data;
  }
